...
const {isTheSameDay} = require('../utils/tools');

let {alreadyCheckIns} = require('../config');
...
module.exports = message => {
  ...
  if (message.MsgType === 'text') {// 文本信息
    ...
    if (message.Content === '签到') {
      if (alreadyCheckIns[options.toUserName] !== 'undefined' && isTheSameDay(alreadyCheckIns[options.toUserName], Date.now())) {
        content = '您今天已经签过到咯！~';
      } else {
        alreadyCheckIns[options.toUserName] = Date.now();
        content = '恭喜你，签到成功！';
      }
